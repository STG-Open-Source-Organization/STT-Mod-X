...
const float shadowFilter1Strength		= 0.00055;
const float shadowFilter2Strength		= 0.00075;
...
float NdotH = dot(normalize(normal),halfAngle);
...
if (Material.landMaterial && !Material.handMaterial) color = mix(color,vec3(-ssaoDarkness),1-ao);
}
...
float xvect = texCoord.x*aspectRatio-vec2(0.5).x*aspectRatio;
float yvect = texCoord.y-vec2(0.5).y;
...
float rFog = clamp(exp(-depth*0.005),0.0,1.0);
...
if (Material.landMaterial && fog) color = mix(vec3(0.8,0.7,0.9),color,rFog);
...
gl_FragData[1] = vec4(vec3(vl+vw+vc+gr,sr,0.0),1.0);
}