...
float t(float v,float t)
 ...
 void main()
 {
   ...
   float G=length(fwidth(b)),L=15.4*1;
   L*=min(g*15.,1.1);
   L*=w.w;
   vec2 W=p(texcoord.xy).xy*.99+.005,H=vec2(0.);
   float q=0.,T=1.1,I=1.,B=16.,V=20.,M=20.,X=mfsXsz(180.,z.totalTexGrad)/(g+.0001),P=mfsXsz(20.,z.totalTexGrad);
   vec4 U=vec4(0.),F=vec4(0.);
   float j=0.;
   vec4 N=vec4(vec3(.25),1.);
   N.xyz=vec3(.5);
   N.xyz*=w.w*.95+.05;
   ...
   for(int A=-1;A<=1;A++)
     {
       for(int Z=-1;Z<=1;Z++)
         {
           vec2 Y=(vec2(A,Z)+H)/vec2(viewWidth,viewHeight)*L,S=texcoord.xy+Y.xy;
           ...
           vec3 O=t(S);
           float K=s(S),J=pow(saturate(dot(D,O)),X),k=exp(-(abs(K-a)*T)),ab=exp(-(s(Q.xyz,w.xyz)*q)),ac=exp(-abs(E-f(S))*P),ad=J*k*ab*ac;
           U+=pow(Q,N)*ad;
           j+=ad;
           F+=Q;
           C++;
