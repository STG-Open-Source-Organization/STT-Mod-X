...
const int shadowMapResolution 			= 2560;
const int ditherPatternRes					= 8;

const float ssaoQuality						= 10.0;
...
const bool timedColors						= true;
const bool vibrantColors						= true;
const bool colorFilter							= true;
const bool contrastedColors				= true;
const bool rainDesaturation				= true;
const bool nightDesaturation				= true;
const bool lowlightDesaturation			= true;
...
godraysSample = isMaterial(texture2D(colortex4,godraysTexCoord).g,0.09,0.21) && eyeIsInWater? 0.0:godraysSample;
...
refGodraysSample = isMaterial(texture2D(colortex4,refGodraysTexCoord).g,0.09,0.21) && eyeIsInWater? 0.0:refGodraysSample;
...
vec3 brightObjects = contrast(saturate(((pow(Material.landMaterial? (Material.handMaterial? color*lightValue*1.1:color):vec3(0.0),vec3(lightSensitivity*1.6)) * (Material.brightMaterials? 2.5:1.0)) * (Material.brightMaterials? 1.0:0.1)),vibrantColors? colorVibrance:1.0),contrastedColors? colorContrast:1.0);
...
brightObjects = colorModifier(brightObjects,1.0,vec3(0.75),Material.brightMaterials? 0.0:lightVisibility,timeMidnight,true);
brightObjects = colorModifier(brightObjects,1.0,vec3(0.75),Material.brightMaterials? 0.0:lightVisibility,rainStrength*(1 - timeMidnight),true);

color = colorModifier(color,1.0,vec3(0.75),lightVisibility,timeMidnight,true);
color = colorModifier(color,1.0,vec3(0.75),lightVisibility,rainStrength*(1 - timeMidnight),true);
