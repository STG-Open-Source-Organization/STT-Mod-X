...
void getVolumetricWaterCaustics(inout vec3 color, inout float wc, inout float vw, in float pixelDepth, in float ditherPattern, in positionStruct Position, in materialStruct Material) {
...
if (Material.landMaterial && fog) color = mix(vec3(0.7,0.7,0.7),color,rFog);
